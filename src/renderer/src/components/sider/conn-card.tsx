  ...
  const handleTraffic = useCallback(
    (_e: unknown, ...args: unknown[]) => {
      ...
      if (platform === 'darwin') {
        if (showTraffic) {
          if (
            info.up === currentUploadRef.current &&
            info.down === currentDownloadRef.current
          )
            return
          currentUploadRef.current = info.up
          currentDownloadRef.current = info.down
          const title = `↑${calcTraffic(info.up)}/s  ↓${calcTraffic(info.down)}/s`
          window.electron.ipcRenderer.send('trayIconUpdate', title, true)
          hasShowTrafficRef.current = true
        } else if (hasShowTrafficRef.current) {
          window.electron.ipcRenderer.send('trayIconUpdate', '', false)
